...
const YearViewModal: React.FC<YearViewModalProps> = ({ year, holidays, onClose, onYearChange }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      clone.style.top = '0';
      clone.style.left = '0'; // Temporarily visible for debugging if needed, but z-index hides it
      clone.style.width = `${posterWidth}px`;
      clone.style.height = 'auto';
      clone.style.minHeight = '100vh';
      clone.style.zIndex = '-9999';
      clone.style.overflow = 'visible';
      clone.style.backgroundColor = '#030508'; // Explicit background color
      clone.style.transform = 'none'; // Reset any transforms

      document.body.appendChild(clone);

      // Wait for images to load if any (though mostly SVG/CSS)
